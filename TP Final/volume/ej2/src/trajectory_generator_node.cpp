...
*/
    ...
    double b0 = ya;
    double b1 = sin(thetaa) * n1 ;
    double b2 = 3 * (yb - ya) - 2 * sin(thetaa) * n1 - sin(thetab) * n2;
    double b3 = -2 * (yb - ya) + sin(thetaa) * n1 + sin(thetab) * n2;
  ...
  {
      // Lados del cuadrado: de (2,2) a (2,-2), a (-2,-2), a (-2,2), a (2,2)
      std::vector<std::tuple<double, double, double>> corners = {{2, 2, M_PI / 2}, {2, -2, 0.0}, {-2, -2, -M_PI / 2.0}, {-2, 2,-M_PI}, {2, 2, - 3.0 * M_PI / 2.0}};
      double current_time = 0.0;
      double desired_speed = 0.2; // m/s

      for (size_t i = 0; i < corners.size() - 1; ++i) {
          double start_x = std::get<0>(corners[i]);
          double start_y = std::get<1>(corners[i]);
          double start_theta = std::get<2>(corners[i]);
          double end_x = std::get<0>(corners[i+1]);
          double end_y = std::get<1>(corners[i+1]);
          double end_theta = std::get<2>(corners[i+1]);
          ...
          for (double d = 0; d < dist; d += stepping) {
              ...
              current_time += stepping / desired_speed; // Incrementamos el tiempo según la velocidad deseada y el paso dado
          }
      }
  }
